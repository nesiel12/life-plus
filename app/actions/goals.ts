...
export async function addGoalAction(title: string, category: LifeAreaKey, milestoneTitles: string[]) {
  const userId = await getCurrentUserId();
  const row = await goalsRepo.createWithMilestones(userId, title, category, milestoneTitles);
  return toGoal(row);
}
