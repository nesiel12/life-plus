...
function analyzeMostActiveDay(activityDates: string[]): PatternCandidate | null {
  ...
  return {
    ...
    subject: "",
    value: String(topDay),
    description: `יום ${DAY_LABELS[topDay]} הוא היום הכי פעיל שלו ב-Life Plus בממוצע.`,
...
function analyzeActivityConsistency(activityDates: string[], now: number): PatternCandidate | null {
  ...
  return {
    ...
    description: `פעיל ב-Life Plus בכ-${Math.round(consistency * 100)}% מהימים ב-30 הימים האחרונים.`,
