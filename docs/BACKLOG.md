...
- **Test coverage is currently just the pure-logic core.** Vitest + CI (lint/typecheck/test on every PR) are live (`.github/workflows/ci.yml`), covering `lib/utils.ts`'s date math, `lib/calendarFreeSlots.ts`, `lib/memory/rankRelevance.ts`, `lib/context/formatContext.ts`, and the chat/context prompt builders. Still open: integration tests for the API routes (auth-required, validation-rejects-bad-input, mock-fallback-when-no-key), a test for `buildAtlasContext` itself (currently untested — thin composition of repo calls, but untested is untested), component tests, and error tracking/structured logging — the rest of Phase 5's exit criterion.
...
- **`personalDNA.peakFocusHours` still doesn't influence calendar-suggestion ranking.** Everything else does now, via the Context Engine (`lib/context/buildAtlasContext.ts`, `docs/ATLAS_ARCHITECTURE_VISION.md` §5) — life-area scores are server-authoritative, and a stale-contact signal (itself derived from `familyCheckInIntervalDays`) enriches a family-category suggestion's rationale. `peakFocusHours` alone is left out because it's free text from onboarding (not structured hours) — using it to filter/prefer time slots would need either a parsing step or folding scheduling through an LLM call, deliberately not done to avoid guessing at a data shape the user never actually committed to. *Files: `app/api/calendar/suggestions/route.ts`.*
