...
- **`buildIntelligenceSignals`/`rankSignals`/`detectPriorityConflicts` have no automated test against the DB-backed orchestration layers that feed them** (`buildAtlasContext`, etc.) — the pure engine itself is fully tested (110 assertions across the suite); what's untested is the same "no integration-test harness yet" gap as everything else DB-touching.
