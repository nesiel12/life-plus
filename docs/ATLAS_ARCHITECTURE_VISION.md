...
**Explicitly still deferred** (audited and named, not silently skipped): a `Modal` focus trap + return-focus-to-trigger on close (all three modal usages — `AICompanion`, `QuickCapture`, `OnboardingFlow` — currently only close on Escape/backdrop, no trapping); a skip-to-content link (low priority given `NavBar` has only 3 short links); Lighthouse performance numbers and a full manual screen-reader pass (both need a live authenticated session this environment can't produce); Timeline/Goals/Learning/Torah/Relationships experience redesigns (each currently functional and calm, but plain — task lists and logs, not journeys); visual-intelligence indicators (pattern/momentum/streak badges) beyond `AIBriefing`'s category icons; any UI for the recommendation-feedback `modified` status, which needs a real edit flow to exist first.
