...
**Explicitly still deferred** (audited and named, not silently skipped): a `Modal` focus trap + return-focus-to-trigger on close (all three modal usages — `AICompanion`, `QuickCapture`, `OnboardingFlow` — currently only close on Escape/backdrop, no trapping); a skip-to-content link (low priority given `NavBar` has only 3 short links); Lighthouse performance numbers and a full manual screen-reader pass (both need a live authenticated session this environment can't produce); Family Care and the areas hub redesigns (currently functional and calm, but plain — task lists and logs, not journeys — Timeline, Goals, and Learning graduated out of this list); the generic `/areas/learning` page (still a plain moments log, distinct from the Torah/Learning workspace above); visual-intelligence indicators beyond `AIBriefing`'s category icons, `GoalJourneyCard`'s stage badges, and `LearningInsightsHero`'s streak; any UI for the recommendation-feedback `modified` status, which needs a real edit flow to exist first.

**Trigger for the next increment:** the same audit discipline applied to Today, Timeline, Goals, and Learning gets applied to one more screen — Family Care is the next natural candidate — once there's real usage data suggesting which of the remaining deferred screens actually needs it most, rather than working the list top-to-bottom by assumption.
