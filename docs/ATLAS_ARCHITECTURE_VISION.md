...
**Explicitly still deferred** (audited and named, not silently skipped): a `Modal` focus trap + return-focus-to-trigger on close (all three modal usages — `AICompanion`, `QuickCapture`, `OnboardingFlow` — currently only close on Escape/backdrop, no trapping); a skip-to-content link (low priority given `NavBar` has only 3 short links); Lighthouse performance numbers and a full manual screen-reader pass (both need a live authenticated session this environment can't produce); Learning/Torah/Relationships experience redesigns (each currently functional and calm, but plain — task lists and logs, not journeys — Timeline and Goals graduated out of this list, see §6 and above); visual-intelligence indicators (pattern/momentum/streak badges) beyond `AIBriefing`'s category icons and `GoalJourneyCard`'s stage badges; any UI for the recommendation-feedback `modified` status, which needs a real edit flow to exist first.

**Trigger for the next increment:** the same audit discipline applied to Today, Timeline, and Goals gets applied to one more screen — Torah Space or Family Care are the next natural candidates (both still task-list/log shaped) — once there's real usage data suggesting which of the remaining deferred screens actually needs it most, rather than working the list top-to-bottom by assumption.
