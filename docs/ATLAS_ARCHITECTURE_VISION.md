...
Current schema (`supabase/migrations/`) — 15 tables, all user-scoped (directly or via a foreign key): `users`, `life_area_scores`, `people`, `moments`, `upcoming_events`, `knowledge_entries` (now with `last_reviewed_at`/`flashcards`/`review_questions` — migration `20260720000006`, Learning Experience v2), `daily_intentions`, `personal_dna`, `goals`, `milestones` (with `completed_at`), `chat_messages`, `insights`, `health_logs`, `personal_patterns`, `recommendation_events`. Every migration through `20260720000005` is applied and verified live; `20260720000006` is written and typed against (`types/database.ts`) but hasn't been run against the real database from this environment yet — see `docs/BACKLOG.md`.
...
- **Recent progress** (`lib/areas/computeActivityTrend.ts`, pure, unit-tested) — there's no score-history table (§8), so this is an honest windowed-count proxy (last 14 days vs. the 14 before that) over `moments`/`knowledge_entries` timestamps that already exist, not a fabricated trend line.
...
**Explicitly still deferred** (audited and named, not silently skipped): a `Modal` focus trap + return-focus-to-trigger on close (all three modal usages — `AICompanion`, `QuickCapture`, `OnboardingFlow` — currently only close on Escape/backdrop, no trapping); a skip-to-content link (low priority given `NavBar` has only 3 short links); Lighthouse performance numbers and a full manual screen-reader pass (both need a live authenticated session this environment can't produce); Family Care (currently functional and calm, but plain — task lists and logs, not journeys — Timeline, Goals, Learning, and Areas have all graduated out of this list); the generic `/areas/learning` page (still a plain moments log, distinct from the Torah/Learning workspace); visual-intelligence indicators beyond `AIBriefing`'s category icons, `GoalJourneyCard`'s stage badges, `LearningInsightsHero`'s streak, and `AreaHealthCard`'s attention badge; any UI for the recommendation-feedback `modified` status, which needs a real edit flow to exist first.

**Trigger for the next increment:** the same audit discipline applied to Today, Timeline, Goals, Learning, and Areas gets applied to the one screen still genuinely plain — Family Care — once there's real usage data suggesting it's actually the highest-value next redesign, rather than working the list top-to-bottom by assumption. For the Companion specifically: real conversation volume that shows the streaming error-handling gap above actually matters in practice, or a concrete need for persisted "based on" history, would each be a deliberate, evidence-gated next step — not built ahead of either.
