      ...
      <div className="flex flex-col gap-6">
        {area && (
          <GlassCard delay={0.05}>
            ...
            <div className="h-1.5 w-full overflow-hidden rounded-full bg-fill-subtle">
              <div
        ...
        <GlassCard delay={0.1}>
          ...
          <textarea
            ...
            rows={2}
            className="focus-ring mb-3 w-full resize-none rounded-lg bg-fill-subtle px-3 py-2 text-sm text-foreground placeholder:text-muted"
          />
          <button
            onClick={handleAdd}
            disabled={!draft.trim() || saving}
            className="rounded-lg bg-fill-subtle px-4 py-2 text-sm text-foreground transition-opacity disabled:opacity-40"
          >
            {saving ? "שומר…" : "שמור"}
          </button>
          ...
          <ul className="flex flex-col gap-3">
            {moments.map((moment, i) => (
              <motion.li
                ...
                className="rounded-xl bg-fill-subtle p-3 text-sm"
              >
